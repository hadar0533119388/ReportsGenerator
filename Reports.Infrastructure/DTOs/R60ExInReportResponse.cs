...
{
    public class R60ExInReportResponse
    {
        ...
        public string SumExReturned => EntryLineMoveList.Sum(item => item.exReturned)?.ToString("N0") ?? string.Empty;

        public string SumNewBalance => EntryLineMoveList.Sum(item => item.NewBalance)?.ToString("N0") ?? string.Empty;




        public R60ExInReportResponse()
        {
